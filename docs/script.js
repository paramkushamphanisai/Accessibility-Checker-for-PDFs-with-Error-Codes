        ...
        tableHtml += `<tr>
            ...
            <td>
                ${res.issues && res.issues.length > 0 
                    ? res.issues.map(i => `<div class="issue-item">• ${i}</div>`).join('')
...
async function analyzePDF(file, idx, total) {
    ...
    // Rule 1: No real text detected
    if (realTextPages === 0) {
        issues.push("No readable text (scanned/image-based PDF)");
    }

    // Rule 2: Too many images
    if (imagePercent >= tolerancePercent) {
        issues.push(`High image content (${Math.round(imagePercent)}%)`);
    }

    // Rule 3: Very low text density
    if (textPercent < 10) {
        issues.push("Very low text content");
    }

    // Rule 4: Possible OCR issue
    if (realTextPages > 0 && totalTextChars < 100) {
        issues.push("Text detected but very limited (possible OCR issue)");
    }

    let accessible = issues.length === 0;
